...
static struct bplus_node *node_seek(struct bplus_tree *tree, off_t offset)
{
	...
	/* tql，居然一定能在cache中找到空闲块

		https://www.zhihu.com/question/269033066/answer/347585034
		节点缓存是以内存缓存磁盘存储的节点，
		将磁盘存储的节点读取到内存，以便改动更新后，写回磁盘。
		实际上MIN_CACHE_NUM为5，
		也就是说我们最多需要五个节点大小的缓存足矣。
	*/
	assert(0);
}
...
int bplus_tree_insert(struct bplus_tree *tree, key_t key, long data)
{
	struct bplus_node *node = node_seek(tree, tree->root);
	while (node != NULL) {
		if (is_leaf(node)) {
...
// only called in bplus_tree_init()，打开/创建索引文件
int bplus_open(char *filename)
{
	return open(filename, O_CREAT | O_RDWR, 0644);
}

// only called in bplus_tree_deinit()，关闭文件
void bplus_close(int fd)
{
	close(fd);
}

// only called in offset_load()
...
// only called int offset_store()
// 不懂作者为什么要把off_t类型的数据格式化成HEX形式的
...
// only called in bplus_tree_init(), 用于读取b+树文件元信息
...
/* 初始化b+树 */
struct bplus_tree *bplus_tree_init(char *filename, int block_size, int key_size)
{
	int i;
	...
	_max_order = (block_size - sizeof(node)) / (key_size + sizeof(off_t));
	_max_entries = (block_size - sizeof(node)) / (key_size + sizeof(long));
	if (_max_order <= 2) {
		fprintf(stderr, "block size is too small for one node!\n");
		return NULL;
	}
	...
	int fd = open(strcat(tree->filename, "_BOOT"), O_RDWR, 0644);
	if (fd >= 0) {
	...
	_max_order = (_block_size - sizeof(node)) / (key_size + sizeof(off_t));
	_max_entries = (_block_size - sizeof(node)) / (key_size + sizeof(long));
...
/* only called in bplus_tree_dump()，与数据可视化有关
   参数只能是内部节点，返回该内点的孩子数 */
...
/* 显示b+树，数据可视化， 参见 https://my.oschina.net/fullofbull/blog/832921 */
